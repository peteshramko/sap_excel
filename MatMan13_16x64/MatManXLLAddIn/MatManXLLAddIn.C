...
__declspec(dllexport) int WINAPI xlAutoAdd(void)
{
	...
	wsprintfW((LPWSTR)szBuf, L"Thank you for adding MatManXLLAddIn.XLL\n build date %hs, time %hs",__DATE__, __TIME__);
...
__declspec(dllexport) int WINAPI xlAutoRemove(void)
{
	...
	Excel12f(xlcAlert, 0, 2, TempStr12(L"Thank you for removing MatManXLLAddIn.XLL!"), TempInt12(2));
	return 1;
}
...
__declspec(dllexport) LPXLOPER12 WINAPI xlAddInManagerInfo12(LPXLOPER12 xAction)
{
	...
	*/
	...
	if(xIntAction.val.w == 1) 
	{
		...
		xInfo.val.str = L"\026MatManXLLAddIn DLL";
	}
	else 
	{
...
__declspec(dllexport) XLOPER12 WINAPI GetStockPrice(char * stockTicker[])
{
	XLOPER12   xRetVal;
	LPWSTR     args[1];
	WCHAR      wticker[256];
	
	if (stockTicker[0] == NULL || strlen(stockTicker[0]) == 0)
		xRetVal.xltype   =  xltypeNum;
		xRetVal.val.num  =  0.00;
		return xRetVal;

	MultiByteToWideChar(CP_ACP, 0, stockTicker[0], -1, wticker, 256);
	args[0] = wticker;
	XLOPER12 result = CallRTD(L"iiiwave.matman.rts.1", L"", 1, args);
	return result;
}
